...
export async function initializeMcpSystem() {
  ...
  try {
    ...
    if (clientsMap.size > 0) {
      logger.info("MCP system already initialized, checking clients...");
      // 检查是否有客户端需要重新初始化
      const config = await getMcpConfigFromFile();
      for (const server of config.servers || []) {
        const clientId = server.id;
        ...
        if (!client || !client.client) {
          logger.info(`Reinitializing client [${clientId}]...`);
...
export async function addMcpServer(clientId: string, config: ServerConfig) {
  try {
    ...
    const isNewServer = !currentConfig.servers?.some(server => server.id === clientId);
    ...
    let newServers = [...(currentConfig.servers || [])];
    if (isNewServer) {
      newServers.push({ ...config, id: clientId });
    } else {
      newServers = newServers.map(server => 
        server.id === clientId ? { ...server, ...config } : server
      );
    }
...
export async function pauseMcpServer(clientId: string) {
  try {
    const currentConfig = await getMcpConfigFromFile();
    const serverConfig = currentConfig.servers?.find(server => server.id === clientId);
    if (!serverConfig) {
    ...
    const newConfig: McpConfigData = {
      ...
      servers: currentConfig.servers?.map(server => 
        server.id === clientId ? { ...server, status: "paused" } : server
      ) || [],
    };
    await updateMcpConfig(newConfig);
  ...
  try {
    ...
    try {
      ...
      const newConfig: McpConfigData = {
        ...currentConfig,
        servers: currentConfig.servers?.map(server => 
          server.id === clientId ? { ...server, status: "active" as const } : server
        ) || [],
      };
      await updateMcpConfig(newConfig);
    } catch (error) {
      const currentConfig = await getMcpConfigFromFile();
      const serverIndex = currentConfig.servers?.findIndex(server => server.id === clientId);

      // 如果配置中存在该服务器，则更新其状态为 error
      if (serverIndex !== undefined && serverIndex >= 0) {
        currentConfig.servers![serverIndex].status = "error";
...
export async function removeMcpServer(clientId: string) {
  try {
    const currentConfig = await getMcpConfigFromFile();
    const newServers = currentConfig.servers?.filter(server => server.id !== clientId) || [];
    const newConfig = {
      ...currentConfig,
      servers: newServers,
    };
...
export async function executeMcpAction(
  ...
  try {
    ...
    if (!client?.client) {
      // 尝试重新初始化客户端
      logger.info(`Client ${clientId} not found, trying to reinitialize...`);
      const config = await getMcpConfigFromFile();
      let serverConfig = config.servers?.find(server => server.id === clientId);
      
      // 如果找不到对应的服务器配置，尝试使用配置文件中的第一个服务器配置
      if (!serverConfig) {
        logger.warn(`Server config for ${clientId} not found, trying to use first available server...`);
        if (config.servers && config.servers.length > 0) {
          const firstServer = config.servers[0];
          if (!firstServer.id) {
            throw new Error(`First server has no id`);
          }
          targetClientId = firstServer.id;
          serverConfig = firstServer;
          logger.info(`Using server ${targetClientId} instead of ${clientId}`);
        } else {
      ...
      await initializeSingleClient(targetClientId, serverConfig);
      client = clientsMap.get(targetClientId);
      if (!client?.client) {
        throw new Error(`Failed to reinitialize client ${targetClientId}`);
      }
    }
...
export async function isMcpEnabled() {
  try {
    ...
    if (typeof process !== "undefined") {
      ...
      // 在客户端，默认返回true，因为我们已经在.env.local中设置了ENABLE_MCP=true
      return true;
    }
  } catch (error) {
